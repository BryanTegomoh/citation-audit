...
| Cross-Paper Attribution | High | Hard | Common (LLM) |
...
## Pattern 26: Cross-Paper Attribution
...
**Risk level**: HIGH. Standard link-checking and DOI validation both pass because all papers are real. Only content-level verification catches this error. Especially common when LLMs or human authors condense findings from multiple papers in the same paragraph.

---

## Pattern 27: Claim-Scope Mismatch

**What it looks like**: The source is real and the URL resolves, but the description of what the source covers overstates or mischaracterizes its actual scope. A paper on "technology and policy implications" gets described as covering "safety and security implications."
...
4. Red flags: overstating a broader scope ("comprehensive review") or a narrower one ("focused exclusively on X") than the source claims
